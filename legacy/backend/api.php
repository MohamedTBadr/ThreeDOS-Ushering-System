...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        $stmt = $connection->prepare("
            INSERT INTO registration (name, email, phone, college, level, event_type, council, ushered_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ");
...
function handleGetRequests($connection, $user)
{
    ...
    foreach (glob(CACHE_DIR . '*.json') as $file) {
        ...
        if ($lastModified === false)
            continue;
        if (time() - $lastModified > CACHE_TTL_DEFAULT)
            @unlink($file);
    }
    ...
    if (isset($_GET['quickstats'])) {
        ...
        if ($cached)
            sendResponse('success', 'Quick stats retrieved (cached)', $cached);
    ...
    if (isset($_GET['interviews'])) {
        ...
        $cached = getCache($cacheKey, 15);
        if ($cached)
            sendResponse('success', 'Interviews retrieved (cached)', $cached);
    ...
    // ------------------------
    if (isset($_GET['quickstats']) || isset($_GET['interviews']))
        return;
    ...
    if ($cached)
        sendResponse('success', 'Applicants retrieved (cached)', $cached);
    ...
    if (!empty($data)) {
        $nextCursor = end($data)['id'];
        $prevCursor = reset($data)['id'];
    }
...
if ($_SERVER["REQUEST_METHOD"] === "PATCH") {
    ...
    if ($user['role'] === 'Instructor')
        $allowed = ['rating', 'notes', 'interview_time', 'interview_questions'];
    elseif ($user['role'] === 'Head' || $user['role'] === "VP")
        $allowed = ['name', 'email', 'phone', 'college', 'level', 'rating', 'notes', 'council', 'interview_time', 'interview_questions', 'ushered_by', 'event_type', 'interviewed_by'];
    elseif ($user['role'] === "OR")
    ...
    foreach ($allowed as $f) {
        if (isset($input[$f])) {
            ...
            if ($f === 'interview_time' && empty($input[$f])) {
                $params[] = null;
                $types .= "s";
            } elseif ($f === 'interview_questions' && is_array($input[$f])) {
